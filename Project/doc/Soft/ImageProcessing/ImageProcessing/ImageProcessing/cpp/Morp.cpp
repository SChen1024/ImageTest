...
QImage Morp::Erode(QImage src, int elem, int kernel, int times)		// 腐蚀
{
	Mat srcImg, dstImg;
...
QImage Morp::Dilate(QImage src, int elem, int kernel, int times)		// 膨胀
{
	Mat srcImg, dstImg;
	srcImg = imgchangeClass->QImage2cvMat(src);
	int dilation_type = 0;
...
QImage Morp::Open(QImage src, int elem, int kernel, int times)		// 开运算
{
	Mat srcImg, dstImg;
	srcImg = imgchangeClass->QImage2cvMat(src);
...
QImage Morp::Close(QImage src, int elem, int kernel, int times)		// 闭运算
{
	Mat srcImg, dstImg;
	srcImg = imgchangeClass->QImage2cvMat(src);
	Mat element = getStructuringElement(elem,Size(2 * kernel + 3, 2 * kernel + 3), Point(kernel + 1, kernel + 1));
...
QImage Morp::Grad(QImage src, int elem, int kernel)		// 形态学梯度
{
...
QImage Morp::Tophat(QImage src, int elem, int kernel)		// 顶帽操作
{
	Mat srcImg, grayImg, dstImg;
	srcImg = imgchangeClass->QImage2cvMat(src);
	Mat element = getStructuringElement(elem,Size(2 * kernel + 3, 2 * kernel + 3), Point(kernel + 1, kernel + 1));
...
QImage Morp::Blackhat(QImage src, int elem, int kernel)	// 黑帽操作
{
	Mat srcImg, grayImg, dstImg;
	srcImg = imgchangeClass->QImage2cvMat(src);
	Mat element = getStructuringElement(elem,Size(2 * kernel + 3, 2 * kernel + 3), Point(kernel + 1, kernel + 1));	
